...
async function createQuestions(id: number) {
  const source = await memorylist();
...
async function createOptions() {
  ...
  await prisma.option.createMany({ data: opts });
}

async function main() {
  await createQuizzes();
  await createQuestions(1);
  await createQuestions(2);
  await createQuestions(3);
  await createQuestions(4);
  await createOptions();
}
