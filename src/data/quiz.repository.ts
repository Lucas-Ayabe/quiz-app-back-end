...
export class QuizRepository {
  ...
  private async createQuizzes() {
    const list = await this.memorylist();
  ...
  private async createQuestions(id: number) {
    const source = await this.memorylist();
    const quiz = await this.prisma.quiz.findFirst({ where: { id } });
    const sourceQuiz = source.find(({ title }) => title === quiz?.title);

    if (quiz && sourceQuiz) {
      await this.prisma.question.createMany({
        data: sourceQuiz.questions.map((question) => ({
  ...
  private async createOptions() {
    const list = await this.memorylist();
    let idx = 0;
    const opts = list.flatMap((quiz) => {
      return quiz.questions.flatMap(({ answer, options }) => {
        const result = options.map((content) => ({
          content,
          questionId: idx + 1,
          ...(answer === content ? { answerForQuestionId: idx + 1 } : {}),
        }));
    ...
    await this.prisma.option.createMany({ data: opts });
  }
